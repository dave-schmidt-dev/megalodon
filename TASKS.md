> **Active plan (narrator summary board — IN PROGRESS 2026-05-24, Phase 1+2 DONE, Phase 3 next):** Phase 1 (`ef4ea18`, narrator client+runtime) and Phase 2 (board_state, NarrativeHub+cache, scheduler, `/api/v1/narrative[-stream]` + gate, `__fake__/narrative` injector) complete on `origin/main`; backend not yet wired into the lifespan (Task 4.1). Each task ran implementer→spec-review→quality-review. Full scripts/tests suite 866 passed. Phase 3 (frontend: board.js, permission_banner extract, terminal drawer, grid removal) is next. `~/Documents/Projects/.plans/megalodon/narrator-summary-board-2026-05-23.md` · tasks: `~/Documents/Projects/.plans/megalodon/narrator-summary-board-2026-05-23-tasks.md` · spec: `docs/superpowers/specs/2026-05-23-narrator-summary-board-design.md`. Replaces the 6-tile grid with a summary-first board (Last/Now/Goal per lane, gemma-e2b narrator runtime, watcher-gated SSE, inline approve/deny, terminal drawer). Warp cycle done (3 reviewers + pre-mortem); 0 escalations. Implementation pending.
>
...
>   - **TODO (lint debt):** 19 pre-existing ruff errors in the tree (6× E741 ambiguous `l`, 7× E402 import-not-at-top incl. test sys.path cases, 4× F841 unused local, 2× E401) — `ruff check megalodon_ui scripts`. The hook lints *staged* files only so these don't block unrelated commits; clean them up (rename `l`, `# noqa: E402` the legitimate sys.path imports, drop unused locals) in a dedicated commit, then the hook can move to whole-tree lint.
>   - **TODO (gate parity, deferred per operator):** pin ruff in CI (`test.yml` uses unpinned `--with ruff`), add a CI dead-code step (CI has none), optionally wire a pre-push hook for the full suite. Tracked, not blocking.
>
