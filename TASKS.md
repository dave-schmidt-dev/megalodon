...
>   - **Finding (HIGH — OPEN follow-up):** `new_run.sh` does not validate the prospective `<run>/.fleet/tmux.sock` path against the 100-byte guard (`_v92_constants.py` `SOCKET_PATH_LIMIT_BYTES`); a long slug fails late at `launch_fleet.sh --spawn` (exit 10). Reject over-long slugs up front with budget math (origin finding: `.archive/2026-05-23T20-24Z--v94h/findings/operator-OPS-new_run-socket-path-no-validation-*.md`). **TODO.**
>
