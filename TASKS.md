...
> - `pending` **P-followup-e2e-1 — grid-smoke inject happy-path broken.** `test_v94_phase1_smoke.spec.ts` A4-A8: `inject-textarea` is never fillable in the chromium-grid-smoke fake-spawner flow (A1–A3 pass; the inject form does not surface the textarea after gotoLaneA). **PRE-EXISTING** — verified to fail identically at baseline 0c21db3, so NOT a regression from the 2026-05-27 suite-health work. Currently `test.fixme`'d to keep `make gate-full` green. Triage: real product bug (inject drawer/modal) vs test-flow drift; fix + un-fixme. files: ui/tests/e2e/test_v94_phase1_smoke.spec.ts, ui/static/pages (lane_detail/inject)
> - `pending` **P-followup-e2e-2 — restart-reconnect self-managed server startup.** `test_restart_reconnect.spec.ts`: the child-process server "never answered /api/v1/config" on the post-restart port. PRE-EXISTING (this project manages its own server, untouched by the suite-health work). Currently `test.fixme`'d. Triage: server-startup timing/flake vs real restart bug; fix + un-fixme. files: ui/tests/e2e/test_restart_reconnect.spec.ts
> - `pending` **P-followup-cov-1 — `scripts/atomic_close.py` 30% coverage.** Atomic claim-close is a correctness path (concurrent lane ownership) sitting far below its peers (`scripts/atomic_close.py:61-172` uncovered). Add unit coverage of the close/race branches. (Surfaced by the Phase-5 usefulness sweep.) files: scripts/atomic_close.py, scripts/tests/
> - `pending` **P-followup-cov-2 — `megalodon_ui/preview.py` 0% coverage.** 68 statements, zero exercise. NOT dead (it's the `python -m megalodon_ui.preview` CLI per README). Add a smoke test of the preview entrypoint (mirror `test_preflight_main.py`). files: megalodon_ui/preview.py, scripts/tests/
>
> ---
>
