> **Active plan: `~/Documents/Projects/.plans/megalodon/governor-hook-permission-architecture-2026-05-25-tasks.md`** (warp-tier). Governor `PreToolUse` hook replaces the screen-scraping permission system; decommissions `permission_watcher.py`. Supersedes the §1b auto-approver. **PHASE 1 COMPLETE 2026-05-25** (subagent-driven: implementer→spec→quality→final, all approved): Task 1.1 `megalodon_ui/governor/policy.py` (pure fail-closed policy engine) + Task 1.2 `megalodon_ui/governor/hook.py` (PreToolUse stdin→decide→stdout + secret-sanitized `.fleet/governor-log` audit). 182 tests (policy 162 / hook 20), ruff + vulture clean. Review caught + closed 4 CRITICAL bypasses (grouping/keyword head-hiding, override cross-segment leak, backslash-newline floor splice, repr-escaped audit-log leak). **Next: Phase 2** — generate `governor-settings.json`, wire `--settings` into all argv paths, preflight + canary + kill-switch.
>
