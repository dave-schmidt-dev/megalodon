...
import { openTerminalModal } from "./terminal_modal.js";
...
// S-HYBRID-DASHBOARD: fetch terminal meta for a lane; returns null on 404 or error.
async function fetchTerminalMeta(short) {
  try {
    const res = await fetch(`/api/v1/lane/${encodeURIComponent(short)}/terminal_meta`);
    if (!res.ok) return null;
    return await res.json();
  } catch (_) {
    return null;
...
// Poll all lanes for terminal meta; update store.termMeta keyed by lane name.
async function pollTerminalMeta(configByLane) {
  const entries = Object.values(configByLane || {});
  if (entries.length === 0) return;
  const current = store.get("termMeta") || {};
  const next = Object.assign({}, current);
  for (const laneConfig of entries) {
    const short = laneConfig?.short;
    const name = laneConfig?.name;
    if (!short || !name) continue;
    const meta = await fetchTerminalMeta(short);
    next[name] = meta;
  }
  store.set("termMeta", next);
}

function fmtCountdown(nextTickUtc) {
  if (!nextTickUtc) return null;
  const tickMs = parseUtcMillis(nextTickUtc);
  if (!Number.isFinite(tickMs)) return null;
  const secondsUntil = Math.floor((tickMs - Date.now()) / 1000);
  if (secondsUntil <= 0) return "tick soon";
  const m = Math.floor(secondsUntil / 60);
  const s = secondsUntil % 60;
  return m > 0 ? `tick in ${m}m${s}s` : `tick in ${s}s`;
}

function renderLaneCard(row, expanded, onToggle, configLane, activitySummary, nextTickUtc, termMeta) {
  const lane = row.lane;
  ...
  // S-NEXT-TICK-VISIBILITY: show countdown to next scheduled wakeup
  const countdownText = fmtCountdown(nextTickUtc);
  if (countdownText) {
    metaChildren.push(el("span", {
      class: "mono",
      style: "opacity:0.55; font-size:11px; color:var(--c-accent,#4a9eff);",
      "data-testid": "lane-next-tick",
      title: nextTickUtc || "",
    }, countdownText));
  }
  const meta = el(
  ...
  // S-HYBRID-DASHBOARD: "View terminal" button — disabled until BE terminal_meta confirms log exists.
  const viewTermBtn = el(
    "button",
    {
      type: "button",
      class: "button",
      ...
      onclick: (ev) => {
        ev.stopPropagation();
        const short = configLane?.short;
        if (short) openTerminalModal({ lane, short });
      },
    },
    "View terminal □"
  );
  ...
  const buttonRow = el(
    "div",
    { class: "row", style: "gap: var(--sp-2); flex-wrap: wrap;" },
    toggleBtn,
    viewTermBtn
  );
...
function renderLaneGrid(container, expanded, onToggle, laneOrder, configByLane) {
  ...
  const termMetaByLane = store.get("termMeta") || {};
  // S-NEXT-TICK-VISIBILITY: build next_tick lookup keyed by lane name
  const nextTickByLane = {};
  for (const ml of (store.get("mission.lanes") || [])) {
  ...
  for (const lane of (laneOrder || LANE_ORDER_FALLBACK)) {
    ...
    const nextTickUtc = nextTickByLane[lane] || null;
    const termMeta = termMetaByLane[lane] || null;
    container.appendChild(renderLaneCard(row, expanded.has(lane), onToggle, configLane, activitySummary, nextTickUtc, termMeta));
  }
}
...
// Parse agent/lane/phase/topic from a finding filename: agent-{id}-{short}-{phase}-{topic}-{UTC}.md
...
// Maps single-letter lane code (from filename) to full lane name for lane-chip CSS class.
const LANE_SHORT_TO_NAME = { A: "AUDIT", B: "ARCHITECT", C: "BACKEND", D: "FRONTEND", E: "TEST", F: "META" };

// Returns a severity-badge CSS class name (must match base.css definitions: BLOCKING/MAJOR/MINOR/NIT/DELTA).
function severityFromTopic(topic) {
  const t = String(topic || "").toLowerCase();
  if (/\b(bug|error|fail|broken|stuck|critical)\b/.test(t)) return "MAJOR";
  if (/\b(idle|scratch|checkpoint)\b/.test(t)) return "DELTA";
  return "NIT";
}
...
function renderHistoryTail(container) {
  ...
  if (recent.length === 0) {
    container.appendChild(el("p", { class: "empty-state" }, "no findings yet — agent findings will appear here"));
    return;
  }
  const ul = el("ul", { class: "stack-1", style: "list-style: none; padding: 0; margin: 0;" });
  for (const f of recent) {
    const utc = utcFromFilename(f.filename) || "—";
    const { agentId, laneShort, phase, topic } = parseFilenameFields(f.filename);
    const sev = String(f.severity || severityFromTopic(topic));
    const laneName = LANE_SHORT_TO_NAME[laneShort] || laneShort;
    const topRow = el("div", { class: "row", style: "gap: var(--sp-2); align-items: center; flex-wrap: wrap;" },
      el("span", { class: `lane-chip ${laneName}` }, laneShort),
      el("span", { class: "mono", style: "font-weight: 600; font-size: 12px; flex: 1;" }, topic),
      el("span", { class: `severity-badge ${sev.toLowerCase()}`, "data-testid": "history-severity-badge" }, sev),
    );
    const metaRow = el("div", { class: "mono", style: "font-size: 11px; opacity: 0.6;" },
      `${agentId} · ${phase} · ${utc}`
    );
    const li = el("li", {
      class: "history-finding-row",
      "data-testid": `history-finding-${f.filename ? f.filename.replace(/\.md$/, "") : "unknown"}`,
      title: f.filename || "",
      role: "button",
      tabindex: "0",
      style: "padding: var(--sp-1) 0; border-bottom: 1px solid var(--c-border, #2a2a2a); cursor: pointer;",
      onclick: () => { window.location.href = "/findings"; },
      onkeydown: (ev) => { if (ev.key === "Enter" || ev.key === " ") { ev.preventDefault(); window.location.href = "/findings"; } },
    }, topRow, metaRow);
    ul.appendChild(li);
  }
  container.appendChild(ul);
...
export async function render(root) {
  ...
  const historyCard = el("section", { class: "card stack-1", "data-testid": "history-tail" },
    el("h2", { class: "card__title" }, "Recent Findings")
  );
  ...
  // S-NEXT-TICK-VISIBILITY: refresh countdown text every 30s so it ticks down
  // between state polls; also subscribe so it re-renders when mission.lanes arrives.
  const countdownTimer = setInterval(() => renderLaneGrid(laneGrid, expanded, onToggle, laneOrder, configByLane), 30_000);
  unsubs.push(store.subscribe("mission.lanes", () => {
    renderLaneGrid(laneGrid, expanded, onToggle, laneOrder, configByLane);
  }));
  ...
  // S-HYBRID-DASHBOARD: poll terminal meta every 5s to enable/disable "View terminal" buttons.
  pollTerminalMeta(configByLane);
  const termMetaTimer = setInterval(() => pollTerminalMeta(configByLane), 5_000);
  unsubs.push(store.subscribe("termMeta", () => {
    renderLaneGrid(laneGrid, expanded, onToggle, laneOrder, configByLane);
  }));
