...
from .constants import (
    ...
    API_LANE_ACTIVITY, API_MISSION_STATUS, API_PHASE_FLIP, API_RECLAIM, API_SIGNAL, API_STATE,
...
# ---------------------------------------------------------------------------
# Stream-log parsing helpers (S-LIVE-ACTIVITY)
# ---------------------------------------------------------------------------
_ANSI_ESC_RE = re.compile(r"\x1b(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])")
_TUI_CTX_RE = re.compile(r"ctx:\s*(\d+[kmKM]?/\d+[kmKM]?)")
_TUI_BOILERPLATE_RE = re.compile(
    r"ctx:\s*\d"
    r"|Sonnet|Haiku|Opus"
    r"|session:\s*\d"
    r"|cache:\s*\d"
    r"|^\s*[─━❯✳✶│]\s*$"
    r"|^\s*/Users/"
    r"|Marinating|Thinking|thinking"
    r"|almost done"
    r"|↓\s*\d"
)
...
def _parse_stream_tail(log_path: Path) -> dict[str, Any]:
    """Read the last _STREAM_TAIL_BYTES of a pipe-pane stream log.

    Returns: last_activity_utc, last_text, token_ctx, status.
    """
    try:
        st = log_path.stat()
    except OSError:
        return {"last_activity_utc": None, "last_text": None, "token_ctx": None, "status": "blocked"}

    age = datetime.now(timezone.utc).timestamp() - st.st_mtime
    status = "active" if age < 30 else ("idle" if age < 300 else "blocked")
    last_activity_utc = (
    ...
    try:
        with log_path.open("rb") as fh:
            fh.seek(max(0, st.st_size - _STREAM_TAIL_BYTES))
    ...
    token_ctx: str | None = None
    for m in _TUI_CTX_RE.finditer(clean):
        token_ctx = m.group(1)  # keep last occurrence

    lines = [ln.strip() for ln in clean.splitlines()]
    meaningful = [
        ln for ln in lines
        if ln and len(ln) > 3 and not _TUI_BOILERPLATE_RE.search(ln)
    ]
    last_text = meaningful[-1] if meaningful else None
...
# ---------------------------------------------------------------------------
# Queue polling helper (BUG-STATUS-NOT-WRITTEN)
# ---------------------------------------------------------------------------


async def _wait_for_queue_result(
    mission_dir: Path, rid: str, timeout: float = 5.0
) -> tuple[str, str | None]:
    """Poll queue dirs until rid is applied/rejected or timeout elapses.

    Returns (status, rejection_reason) where status ∈ {applied, rejected, pending}.
    Uses asyncio.sleep so the event loop stays live between polls.
    """
    loop = asyncio.get_running_loop()
    deadline = loop.time() + timeout
    while True:
        if (mission_dir / "queue" / "applied" / f"{rid}.json").exists():
            return "applied", None
        reason_file = mission_dir / "queue" / "rejected" / f"{rid}-reason.txt"
        if (mission_dir / "queue" / "rejected" / f"{rid}.json").exists():
...
def _register_routes(app: FastAPI, ctx: MissionContext) -> None:
    ...
    async def get_v1_state():
        ...
        if events_path.exists():
            try:
                ...
                pass
        # S-NEXT-TICK-VISIBILITY: read per-lane next_tick from .fleet/<short>.next_tick.txt
        mission_lanes: list[dict[str, Any]] = []
        if ctx.mission_config:
    ...
    @app.get(API_LANE_ACTIVITY)
    async def get_lane_activity_summary(short: str):
        """S-LIVE-ACTIVITY — per-lane pipe-pane stream tail summary.

        Reads .fleet/{short}.stream.log, strips ANSI, parses Claude TUI
        footer for token context, determines status from mtime.
        """
        ...
        if not log_path.exists():
            raise HTTPException(status_code=404, detail=f"stream log for lane {short!r} not found")
        return _parse_stream_tail(log_path)
    ...
    # ---------------------------------------------------------------------------
    # Agent-facing auth + queue proxy endpoints (v9.3 BUG-STATUS-NOT-WRITTEN)
    # ---------------------------------------------------------------------------
    # These endpoints are called by /loop agents via curl.  They bridge the
    # HTTP request/response world to the file-based queue that the applier
    # drains.  The ?wait=true variant polls for up to ~5 s so a single curl
    # call gets the final outcome without a polling loop in shell.
    # ---------------------------------------------------------------------------

    def _session_token() -> str:
        """Read the current bearer token from .fleet/ui.token (hot-reads on each call)."""
        try:
            return (ctx.mission_dir / ".fleet" / "ui.token").read_text().strip()
        except OSError:
            return ""

    def _check_session(request: Request) -> None:
        """Raise 401 if the request lacks a valid session cookie."""
        expected = _session_token()
        if not expected:
            raise HTTPException(status_code=500, detail="token file not readable")
        if request.cookies.get("megalodon_session", "") != expected:
            raise HTTPException(status_code=401, detail="authentication required")

    @app.post("/api/v1/auth/exchange")
    async def post_v1_auth_exchange(req: Request) -> Response:
        """Exchange bearer token for a session cookie.

        Body: {token: "<bearer-token>"}
        Sets megalodon_session cookie on success.
        """
        body = await req.json()
        ...
        resp.set_cookie("megalodon_session", token, httponly=True, samesite="strict")
        return resp

    def _queue_response(
        rid: str, intent: str, status: str, reason: str | None
    ) -> JSONResponse:
        code = 200 if status == "applied" else (409 if status == "rejected" else 202)
        return JSONResponse(
            status_code=code,
            content={"request_id": rid, "intent": intent, "status": status,
                     "rejection_reason": reason},
        )

    @app.post("/api/v1/status/update")
    async def post_v1_status_update(req: Request, wait: str | None = None):
        """Update a STATUS.md row for the given lane via queue.

        Body: {lane, agent, new_state, new_utc?, new_notes?}
        ?wait=true: block up to ~5 s; returns 200/409/202.
        """
        _check_session(req)
        body = await req.json()
        lane = str(body.get("lane", "")).strip().upper()
        agent = str(body.get("agent", "")).strip()
        new_state = str(body.get("new_state", "")).strip()
        new_utc = str(body.get("new_utc") or "").strip() or None
        new_notes = str(body.get("new_notes") or "").strip()
        if not lane or not agent or not new_state:
            raise HTTPException(status_code=422, detail="lane, agent, new_state required")
        rid = _qc.status_update(
            ctx.mission_dir, agent=agent, lane=lane,
            new_state=new_state, new_utc=new_utc, new_notes=new_notes,
        )
        if wait and wait.lower() in ("true", "1"):
            status, reason = await _wait_for_queue_result(ctx.mission_dir, rid)
            return _queue_response(rid, "STATUS_UPDATE", status, reason)
        return JSONResponse(
            status_code=202,
            content={"request_id": rid, "intent": "STATUS_UPDATE", "status": "pending"},
            headers={"Location": f"/api/v1/queue/{rid}"},
    ...
    async def post_v1_task_claim(req: Request, wait: str | None = None):
        """Mark a task as claimed in TASKS.md via queue.

        Body: {lane, task_id, agent}
        ?wait=true: block up to ~5 s; returns 200/409/202.
        """
        _check_session(req)
        body = await req.json()
        ...
        agent = str(body.get("agent", "")).strip()
        if not lane or not task_id or not agent:
        ...
        if wait and wait.lower() in ("true", "1"):
            status, reason = await _wait_for_queue_result(ctx.mission_dir, rid)
            return _queue_response(rid, "TASKS_BRACKET", status, reason)
        return JSONResponse(
            status_code=202,
            content={"request_id": rid, "intent": "TASKS_BRACKET", "status": "pending"},
            headers={"Location": f"/api/v1/queue/{rid}"},
        )

    @app.post("/api/v1/task/done")
    async def post_v1_task_done(req: Request, wait: str | None = None):
        """Mark a task as done in TASKS.md via queue.
    ...
    async def post_v1_history_append(req: Request, wait: str | None = None):
        """Append a completion line to HISTORY.md via queue.

        Body: {lane, agent, task_id, finding_path, severity?}
        ?wait=true: block up to ~5 s; returns 200/409/202.
        """
        _check_session(req)
        body = await req.json()
        ...
        severity = str(body.get("severity") or "INFO").strip().upper()
        if not lane or not agent or not task_id or not finding_path:
        ...
        if wait and wait.lower() in ("true", "1"):
            status, reason = await _wait_for_queue_result(ctx.mission_dir, rid)
            return _queue_response(rid, "HISTORY_APPEND", status, reason)
        return JSONResponse(
            status_code=202,
            content={"request_id": rid, "intent": "HISTORY_APPEND", "status": "pending"},
            headers={"Location": f"/api/v1/queue/{rid}"},
        )
