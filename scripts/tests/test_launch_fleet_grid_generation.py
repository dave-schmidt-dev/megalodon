...
def test_dry_run_queue_mission_six_lanes():
    ...
    assert len(lane_lines) == 6, (
        f"Expected 6 lane= lines, got {len(lane_lines)}: {r.stdout}"
    )
...
def test_dry_run_minimal_3_lane_cr4_banners():
    ...
    assert len(banner_lines) == 2, (
        f"Expected 2 MANUAL TICK banners, got {len(banner_lines)}"
    )
    ...
    assert "MANUAL TICK" not in lines[alpha_idx], (
        "ALPHA should not have banner on its own line"
    )
    if alpha_idx + 1 < len(lines):
        assert "MANUAL TICK" not in lines[alpha_idx + 1], (
            "Line after ALPHA should not be a banner"
        )

    # BETA and GAMMA should be followed by banners
    beta_idx = next(i for i, ln in enumerate(lines) if "lane=BETA" in ln)
    assert "MANUAL TICK" in lines[beta_idx + 1], (
        "BETA should be followed by MANUAL TICK banner"
    )

    gamma_idx = next(i for i, ln in enumerate(lines) if "lane=GAMMA" in ln)
    assert "MANUAL TICK" in lines[gamma_idx + 1], (
        "GAMMA should be followed by MANUAL TICK banner"
    )
