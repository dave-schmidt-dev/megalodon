...
test.describe('lane_detail: inject form send flow', () => {
  ...
  test('type "hello world" → click Send → POST /api/v1/lane/A/inject with correct body and X-CSRF-Token', async ({ page }) => {
...
test.describe('lane_detail: byte limit enforcement', () => {
  ...
  test('type 16385 chars → Send button disabled + warning visible', async ({ page }) => {
    await gotoLaneA(page);
  ...
  test('type exactly 16384 chars → Send button enabled and no warning', async ({ page }) => {
    await gotoLaneA(page);
