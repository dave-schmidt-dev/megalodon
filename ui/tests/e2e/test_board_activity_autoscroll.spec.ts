...
function writeFinding(fixtureRoot: string, filename: string, lane: string): void {
...
test('I3: a scrolled-back reader is NOT yanked to the top on a new event', async ({ page }, testInfo) => {
  ...
  for (let i = 0; i < 40; i++) {
    writeFinding(fixtureRoot, `agent-scroll-A-${ts}-${i}.md`, 'A');
  }
  ...
  writeFinding(fixtureRoot, `agent-scroll-A-${ts}-after.md`, 'A');
...
test('I3: a reader at the top DOES follow new events (auto-scroll preserved at top)', async ({ page }, testInfo) => {
  ...
  for (let i = 0; i < 40; i++) {
    writeFinding(fixtureRoot, `agent-attop-A-${ts}-${i}.md`, 'A');
  }
  await page.waitForTimeout(700);
  ...
  writeFinding(fixtureRoot, `agent-attop-A-${ts}-after.md`, 'A');
  await page.waitForTimeout(900);
